...
export class AlertResolverBase {
  ...
  async createAlert(@graphql.Args() args: CreateAlertArgs): Promise<Alert> {
    return await this.service.createAlert({
      ...
      data: {
        ...
        networkEvent: args.data.networkEvent
          ? {
              connect: args.data.networkEvent,
            }
          : undefined,
      },
    });
  ...
  @graphql.ResolveField(() => NetworkEvent, {
    nullable: true,
    name: "networkEvent",
  })
  async getNetworkEvent(
    @graphql.Parent() parent: Alert
  ): Promise<NetworkEvent | null> {
    const result = await this.service.getNetworkEvent(parent.id);
